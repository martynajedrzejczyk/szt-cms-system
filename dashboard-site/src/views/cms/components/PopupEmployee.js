...
const PopupEmployee = ({ id, name, surname, cityId, description, visible, image, cities, closePopup, changeData }) => {
    const [newName, setNewName] = React.useState(name);
    const [newSurname, setNewSurname] = React.useState(surname);
    const [newCity, setNewCity] = React.useState(cities.find((city) => city._id === cityId)._id);
    ...
    return (
        <div className="popup-container">
            ...
            <CRow className="mb-3 popup-buttons">
                <CButton color="primary" className="col-sm-2" onClick={() => changeData(id, newName, newSurname, newCity, newDescription, newVisibility, newImage)}>Zapisz</CButton>
